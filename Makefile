...
clean :
	...
	rm -rf printbig
	rm -rf open
	rm -rf *.cmx *.cmi *.cmo *.cmx *.o *.s *.ll *.out *.exe *.*.depends
...
TESTS = add1 arith1 arith2 arith3 fib for1 for2 func1 func2 func3	\
    ...
    while1 while2 printbig helloworld open
...
TARFILES = ast.ml codegen.ml Makefile _tags microc.ml parser.mly README \
        scanner.mll semant.ml testall.sh printbig.c open.c printarcade-font.pbm font2c \
	$(TESTFILES:%=tests/%)
